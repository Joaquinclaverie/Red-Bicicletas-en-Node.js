        ...
        Usuario.findOne({email:req.body.email}, function(err, userInfo){
            if (err) {
                ...
                if (userInfo != null && bcrypt.compareSync(req.body.password, userInfo.password)) {
                    ...
                    res.status(401).json({status:"error", message: "Email/password invalido", data:null});
                }
            }
        });
    },
    forgotPassoword: function(req, res, next){
        Usuario.findOne({ email: req.body.email }, function(err, usuario) {
            if(!usuario) return res.status(401).json({ message: "No existe el usuario", })
            usuario.resetPassword(function(err){
                if (err) { return next(err); }
                res.status(200).json({message: "Se envio un email para reestablecer el "})
            });
        });
    },
}
