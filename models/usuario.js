...
const validateEmail = function(email) {
    const re = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i; //regex para validar un email /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
...
usuarioSchema.methods.resetPassword = function(cb) {
    ...
    token.save(function (err) {
        ...
        mailer.sendMail(mailOptions, function (err) {
            ...
            console.log('Se ha enviado un email para resetear la password a: ' + email_destination + '.');
        });
...
usuarioSchema.statics.findOneOrCreateByFacebook = function findOrCreate(condition, callback) {
    const self = this;
    console.log(condition);
    self.findOne({
        $or:[
